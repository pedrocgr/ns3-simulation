...
def plot_comparison(results_dir: Path):
    ...
    plt.plot(reno_t_x, reno_t_y, label="TCP Reno", color="#1f77b4", linewidth=1.8)
    plt.title("Atividade 4 - Throughput TCP Reno no Tempo")
    plt.xlabel("Tempo (s)")
    plt.ylabel("Throughput (Mbps)")
    plt.grid(True, linestyle="--", alpha=0.4)
    plt.legend()
    plt.tight_layout()
    plt.savefig(results_dir / "activity2-reno-throughput.png", dpi=150)
    plt.close()

    plt.figure(figsize=(10, 5))
    plt.plot(cubic_t_x, cubic_t_y, label="TCP Cubic", color="#d62728", linewidth=1.8)
    plt.title("Atividade 5 - Throughput TCP Cubic no Tempo")
    plt.xlabel("Tempo (s)")
    plt.ylabel("Throughput (Mbps)")
    plt.grid(True, linestyle="--", alpha=0.4)
    plt.legend()
    plt.tight_layout()
    plt.savefig(results_dir / "activity2-cubic-throughput.png", dpi=150)
    plt.close()
...
if __name__ == "__main__":
    ...
    print("Generated:")
    print(results / "activity2-reno-throughput.png")
    print(results / "activity2-cubic-throughput.png")
